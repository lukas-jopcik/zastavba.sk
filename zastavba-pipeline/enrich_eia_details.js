...
/* Usage:
   ...
   Voliteľne zapis do DB po každom batche:
   WRITE_DB=1 node enrich_eia_details.js data_raw/2025-08-28_eia.json
*/
...
(async () => {
  ...
  try {
    ...
    const agree = p.locator('button:has-text("Súhlas"), button:has-text("Súhlasím"), button:has-text("Akceptujem"), button:has-text("Prijať")').first();
  ...
  for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
    ...
    // 2) voliteľne: normalize + geocode + DB upsert po batche
    if (WRITE_DB) {
      try {
        const withGeo = await normalizeAndGeocode(results);
        let ok = 0, fail = 0;
        for (const row of withGeo) {
          try {
            ...
            fail++;
            console.warn('   DB upsert failed for', row.detail_url, e.message);
          }
        }
        console.log(`   💾 DB upsert batch=${batchIdx + 1}: ok=${ok}, fail=${fail}`);
      } catch (e) {
        console.warn('   ⚠️ normalizeAndGeocode failed:', e.message);
      }
    }
  }
